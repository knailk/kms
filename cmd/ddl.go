...
func PSQLArgs(up bool) ([]string, error) {
	...
	// config.Init will retrieve the database info from config file
	cfg, err := config.Init()
	if err != nil {
		return nil, errs.E(op, err)
	}
	...
	args := []string{"-w", "-d", config.NewPostgreSQLDSN(cfg).ConnectionURI(), "-c", "select current_database(), current_user, version()"}
